...
static bool ft_more_stars(char **pattern, char *last_wildcard, char **last, char *str)
{
    while (**pattern == '*')
        (*pattern)++;
    if (!**pattern)
...
static bool ft_match_pattern(char **pattern, char *last_wildcard, char **last, char *str)
{
    if (**pattern == **str)
    {
        (*pattern)++;
...
bool    ft_star_match(char **pattern, char *str)
{
    ...
    while(*str)
    {
        ...
        if (*pattern == '*' && !quotes && ft_more_stars(&pattern, &last_wildcard, &last, str))
            return (true);
